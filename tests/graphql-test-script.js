...
const GRAPHQL_URL = "http://40.69.163.167:8000/graphql";
...
// Mutation metrics
let responseTimeMutationCreateAssignment = new Trend("response_time_create_assignment");
let responseTimeMutationCreateCourse = new Trend("response_time_create_course");
let responseTimeMutationCreateEnrollment = new Trend("response_time_create_enrollment");
let responseTimeMutationCreateRole = new Trend("response_time_create_role");
let responseTimeMutationCreateSection = new Trend("response_time_create_section");
let responseTimeMutationUpdateAssignment = new Trend("response_time_update_assignment");
let responseTimeMutationUpdateCourse = new Trend("response_time_update_course");
let responseTimeMutationUpdateEnrollment = new Trend("response_time_update_enrollment");
let responseTimeMutationUpdateSection = new Trend("response_time_update_section");
let responseTimeMutationDeleteEnrollment = new Trend("response_time_delete_enrollment");
let responseTimeMutationDeleteRole = new Trend("response_time_delete_role");
let responseTimeMutationDeleteSection = new Trend("response_time_delete_section");
let responseTimeMutationLogin = new Trend("response_time_login");
let responseTimeMutationChangePassword = new Trend("response_time_change_password");
...
const mutations = {
  ...
  createAssignment: (input) => `mutation {
    createAssignment(input: {
      ...
      allowsubmissionsfromdate: ${input.allowsubmissionsfromdate ? `"${input.allowsubmissionsfromdate.toISOString()}"` : "null"},
  ...
  createEnrollment: (input) => `mutation {
    createEnrollment(input: {
      ...
      status: ${input.status},
      timestart: ${input.timestart ? `"${input.timestart.toISOString()}"` : "null"},
...
function testGraphQLQueries() {
  ...
  responses.home = http.get("http://40.69.163.167:8000/");
  ...
  if (Math.random() < 0.3) {
    responses.userById = executeGraphQLQuery(queries.user(testData.existingUserId));
    responses.roleById = executeGraphQLQuery(queries.role(randomIntBetween(1, 5)));
    responses.courseById = executeGraphQLQuery(queries.course(testData.existingCourseId));
    responses.courseSections = executeGraphQLQuery(queries.courseSections(testData.existingCourseId));
    responses.cursoAssignments = executeGraphQLQuery(queries.courseAssignmentsProx(testData.existingCourseId));
    responses.seccionAssignments = executeGraphQLQuery(queries.assignments(testData.existingCourseId, testData.existingSectionId));
    responses.allAssignments = executeGraphQLQuery(queries.allAssignments);
    responses.enrollmentsByCourse = executeGraphQLQuery(queries.courseEnrollments(testData.existingCourseId));
    responses.enrollmentsByUser = executeGraphQLQuery(queries.userEnrollments(testData.existingUserId));
    responses.submissionsByAssignment = executeGraphQLQuery(queries.submissions(testData.existingAssignmentId));
    responses.submissionsByUser = executeGraphQLQuery(queries.userSubmissions(testData.existingUserId));
    responses.sections = executeGraphQLQuery(queries.sections(testData.existingCourseId));
  }
  ...
  let checks = {
    ...
    "users query successful": (r) => {
      ...
      return responses.users.status === 200 && !body.errors && body.data && body.data.users;
    },
    "categories query successful": (r) => {
      let body = JSON.parse(responses.categories.body);
      return responses.categories.status === 200 && !body.errors && body.data && body.data.categories;
    },
    "roles query successful": (r) => {
      let body = JSON.parse(responses.roles.body);
      return responses.roles.status === 200 && !body.errors && body.data && body.data.roles;
    },
    "courses query successful": (r) => {
      let body = JSON.parse(responses.courses.body);
      return responses.courses.status === 200 && !body.errors && body.data && body.data.courses;
    },
    "assignments query successful": (r) => {
      let body = JSON.parse(responses.assignments.body);
      return responses.assignments.status === 200 && !body.errors && body.data && body.data.assignments !== undefined;
    },
    "assignmentsProx query successful": (r) => {
      let body = JSON.parse(responses.assignmentsProx.body);
      return responses.assignmentsProx.status === 200 && !body.errors && body.data && body.data.AllAssigmentsProx !== undefined;
    }
  };
  ...
  if (responses.userById) {
    checks["userById query successful"] = (r) => {
      ...
      return responses.userById.status === 200 && !body.errors && body.data && body.data.user;
    };
  }
  ...
  if (responses.roleById) {
    checks["roleById query successful"] = (r) => {
      ...
      return responses.roleById.status === 200 && !body.errors && body.data && body.data.role;
    };
  }
  ...
  if (responses.courseById) {
    checks["courseById query successful"] = (r) => {
      ...
      return responses.courseById.status === 200 && !body.errors && body.data && body.data.course;
    };
  }
  ...
  if (responses.courseSections) {
    checks["courseSections query successful"] = (r) => {
      ...
      return responses.courseSections.status === 200 && !body.errors && body.data && body.data.courseSections !== undefined;
    };
  }
  ...
  if (responses.cursoAssignments) {
    checks["cursoAssignments query successful"] = (r) => {
      ...
      return responses.cursoAssignments.status === 200 && !body.errors && body.data && body.data.CourseAssignmentsProx !== undefined;
    };
  }
  ...
  if (responses.seccionAssignments) {
    checks["seccionAssignments query successful"] = (r) => {
      ...
      return responses.seccionAssignments.status === 200 && !body.errors && body.data && body.data.assignments !== undefined;
    };
  }
  ...
  if (responses.allAssignments) {
    checks["allAssignments query successful"] = (r) => {
      ...
      return responses.allAssignments.status === 200 && !body.errors && body.data && body.data.AllAssigments !== undefined;
    };
  }
  ...
  if (responses.enrollmentsByCourse) {
    checks["enrollmentsByCourse query successful"] = (r) => {
      ...
      return responses.enrollmentsByCourse.status === 200 && !body.errors && body.data && body.data.courseEnrollments !== undefined;
    };
  }
  ...
  if (responses.enrollmentsByUser) {
    checks["enrollmentsByUser query successful"] = (r) => {
      ...
      return responses.enrollmentsByUser.status === 200 && !body.errors && body.data && body.data.userEnrollments !== undefined;
    };
  }
  ...
  if (responses.submissionsByAssignment) {
    checks["submissionsByAssignment query successful"] = (r) => {
      ...
      return responses.submissionsByAssignment.status === 200 && !body.errors && body.data && body.data.submissions !== undefined;
    };
  }
  ...
  if (responses.submissionsByUser) {
    checks["submissionsByUser query successful"] = (r) => {
      ...
      return responses.submissionsByUser.status === 200 && !body.errors && body.data && body.data.userSubmissions !== undefined;
    };
  }
  ...
  if (responses.sections) {
    checks["sections query successful"] = (r) => {
      ...
      return responses.sections.status === 200 && !body.errors && body.data && body.data.sections !== undefined;
    };
  }
  ...
  if (responses.roleById) responseTimeTrendRolById.add(responses.roleById.timings.duration);
  ...
  if (responses.sections) responseTimeTrendSections.add(responses.sections.timings.duration);
...
function testGraphQLCreateMutations() {
  ...
  if (Math.random() < 0.3) {
    responses.createAssignment = executeGraphQLQuery(mutations.createAssignment(assignmentInput));
  }
  
  if (Math.random() < 0.2) {
    responses.createCourse = executeGraphQLQuery(mutations.createCourse(courseInput));
  }
  
  if (Math.random() < 0.3) {
    responses.createEnrollment = executeGraphQLQuery(mutations.createEnrollment(enrollmentInput));
  }
  ...
  if (Math.random() < 0.3) {
    responses.createSection = executeGraphQLQuery(mutations.createSection(sectionInput));
  }
  ...
  if (responses.createAssignment) {
    checks["create assignment successful"] = (r) => {
      ...
      if (responses.createAssignment.status === 200 && !body.errors && body.data && body.data.createAssignment) {
        testData.createdAssignmentId = body.data.createAssignment.id || testData.existingAssignmentId;
        return true;
      }
      return false;
    };
  ...
  if (responses.createCourse) {
    checks["create course successful"] = (r) => {
      ...
      if (responses.createCourse.status === 200 && !body.errors && body.data && body.data.createCourse) {
        testData.createdCourseId = body.data.createCourse.id || testData.existingCourseId;
        return true;
      }
      return false;
    };
  ...
  if (responses.createEnrollment) {
    checks["create enrollment successful"] = (r) => {
      ...
      if (responses.createEnrollment.status === 200 && !body.errors && body.data && body.data.createEnrollment) {
  ...
  if (responses.createRole) {
    checks["create role successful"] = (r) => {
      ...
      if (responses.createRole.status === 200 && !body.errors && body.data && body.data.createRole) {
  ...
  if (responses.createSection) {
    checks["create section successful"] = (r) => {
      ...
      if (responses.createSection.status === 200 && !body.errors && body.data && body.data.createSection) {
        testData.createdSectionId = body.data.createSection.id || testData.existingSectionId;
        return true;
      }
      return false;
    };
  ...
  if (responses.createAssignment) responseTimeMutationCreateAssignment.add(responses.createAssignment.timings.duration);
  if (responses.createCourse) responseTimeMutationCreateCourse.add(responses.createCourse.timings.duration);
  if (responses.createEnrollment) responseTimeMutationCreateEnrollment.add(responses.createEnrollment.timings.duration);
  if (responses.createRole) responseTimeMutationCreateRole.add(responses.createRole.timings.duration);
  if (responses.createSection) responseTimeMutationCreateSection.add(responses.createSection.timings.duration);
...
function testGraphQLUpdateMutations() {
  ...
  if (Math.random() < 0.25) {
    responses.updateAssignment = executeGraphQLQuery(mutations.updateAssignment(targetId, assignmentInput));
  }
  ...
  if (Math.random() < 0.25) {
    responses.updateCourse = executeGraphQLQuery(mutations.updateCourse(targetId, courseInput));
  }
  ...
  if (testData.createdEnrollmentId > 0 && Math.random() < 0.25) {
    ...
    if (Math.random() < 0.25) {
      responses.updateEnrollment = executeGraphQLQuery(mutations.updateEnrollment(targetId, enrollmentInput));
    }
  }
  ...
  if (testData.createdRoleId > 0 && Math.random() < 0.25) {
    ...
    if (Math.random() < 0.25) {
      responses.updateRole = executeGraphQLQuery(mutations.updateRole(targetId, roleInput));
    }
  }
  ...
  if (Math.random() < 0.25) {
    responses.updateSection = executeGraphQLQuery(mutations.updateSection(targetId, sectionInput));
  }
  ...
  if (responses.updateAssignment) {
    checks["update assignment successful"] = (r) => {
      ...
      return responses.updateAssignment.status === 200 && !body.errors && body.data && body.data.updateAssignment;
    };
  }
  ...
  if (responses.updateCourse) {
    checks["update course successful"] = (r) => {
      ...
      return responses.updateCourse.status === 200 && !body.errors && body.data && body.data.updateCourse;
    };
  }
  ...
  if (responses.updateEnrollment) {
    checks["update enrollment successful"] = (r) => {
      ...
      return responses.updateEnrollment.status === 200 && !body.errors && body.data && body.data.updateEnrollment;
    };
  }
  ...
  if (responses.updateRole) {
    checks["update role successful"] = (r) => {
      ...
      return responses.updateRole.status === 200 && !body.errors && body.data && body.data.updateRole;
    };
  }
  ...
  if (responses.updateSection) {
    checks["update section successful"] = (r) => {
      ...
      return responses.updateSection.status === 200 && !body.errors && body.data && body.data.updateSection;
    };
  }
  ...
  // Registrar métricas de tiempo de respuesta
  if (responses.updateAssignment) responseTimeMutationUpdateAssignment.add(responses.updateAssignment.timings.duration);
  if (responses.updateCourse) responseTimeMutationUpdateCourse.add(responses.updateCourse.timings.duration);
  if (responses.updateEnrollment) responseTimeMutationUpdateEnrollment.add(responses.updateEnrollment.timings.duration);
  if (responses.updateSection) responseTimeMutationUpdateSection.add(responses.updateSection.timings.duration);
...
function testGraphQLDeleteMutations() {
  ...
  if (testData.createdEnrollmentId > 0 && Math.random() < 0.2) {
    responses.deleteEnrollment = executeGraphQLQuery(mutations.deleteEnrollment(testData.createdEnrollmentId));
    testData.createdEnrollmentId = 0; // Limpiamos el ID después de eliminar
  }
  
  if (testData.createdRoleId > 0 && Math.random() < 0.2) {
    responses.deleteRole = executeGraphQLQuery(mutations.deleteRole(testData.createdRoleId));
    testData.createdRoleId = 0;
  }
  
  if (testData.createdSectionId > 0 && Math.random() < 0.2) {
    responses.deleteSection = executeGraphQLQuery(mutations.deleteSection(testData.createdSectionId));
  ...
  if (responses.deleteEnrollment) {
    checks["delete enrollment successful"] = (r) => {
      ...
      return responses.deleteEnrollment.status === 200 && !body.errors && body.data && body.data.deleteEnrollment;
    };
  }
  ...
  if (responses.deleteRole) {
    checks["delete role successful"] = (r) => {
      ...
      return responses.deleteRole.status === 200 && !body.errors && body.data && body.data.deleteRole;
    };
  }
  ...
  if (responses.deleteSection) {
    checks["delete section successful"] = (r) => {
      ...
      return responses.deleteSection.status === 200 && !body.errors && body.data && body.data.deleteSection;
    };
  }
  ...
  // Registrar métricas de tiempo de respuesta
  if (responses.deleteEnrollment) responseTimeMutationDeleteEnrollment.add(responses.deleteEnrollment.timings.duration);
  if (responses.deleteRole) responseTimeMutationDeleteRole.add(responses.deleteRole.timings.duration);
  if (responses.deleteSection) responseTimeMutationDeleteSection.add(responses.deleteSection.timings.duration);
